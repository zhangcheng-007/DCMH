...
def split_data(images, tags, labels):
    ...
    Y = {}
    Y['query'] = tags[:,0: opt.query_size].transpose()
    Y['train'] = tags[:,opt.query_size: opt.training_size + opt.query_size].transpose()
    Y['retrieval'] = tags[:,opt.query_size: opt.query_size + opt.database_size].transpose()

    L = {}
    L['query'] = labels[:,0: opt.query_size].transpose()
    L['train'] = labels[:,opt.query_size: opt.training_size + opt.query_size].transpose()
    L['retrieval'] = labels[:,opt.query_size: opt.query_size + opt.database_size].transpose()
